...
public class EntregaDaoImpl implements EntregaDao {
    ...
    public Entrega crear(Entrega entrega) {
        // Corregimos los nombres de las columnas para que coincidan con tu DB
        String sql = "INSERT INTO Entrega (id_contrato, url_archivo, descripcion, estado) VALUES (?, ?, ?, 'PENDIENTE')";
        try (Connection conn = ConexionDB.getConnection();
             PreparedStatement stmt = conn.prepareStatement(sql, Statement.RETURN_GENERATED_KEYS)) {
            
            stmt.setInt(1, entrega.getIdContrato());
            stmt.setString(2, entrega.getUrlArchivo());
            stmt.setString(3, entrega.getDescripcion());
        ...
        try {
            ...
            try (PreparedStatement stmtInfo = conn.prepareStatement(sqlInfo)) {
                ...
                try (ResultSet rs = stmtInfo.executeQuery()) {
                    if (!rs.next()) throw new Exception("La entrega no existe.");
                    
                    clienteDuenio = rs.getInt("id_cliente");
                    if (clienteDuenio != idCliente) throw new Exception("No tienes permiso para evaluar esta entrega.");
    ...
    @Override public Optional<Entrega> obtenerPorId(Integer id) { return Optional.empty(); }
    @Override public List<Entrega> obtenerTodos() { return null; }
    @Override public boolean actualizar(Entrega entidad) { return false; }
